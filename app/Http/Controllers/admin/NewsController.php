...
class NewsController extends VoyagerBaseController
{
    ...
    {
        $all_news = News::pluck('title', 'id')->toArray();
        $thisItem = News::find($id);
        $similar_news_array = SimilarNews::where('news_id', $id)->get();
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'all_news', 'thisItem', 'similar_news_array'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->input('news_id')) {
            ...
            $detach_similar_newsId = DB::table('similar_news')
                                     ->select()
                                     ->where('news_id', $data->id)
                                     ->get();
            $similar_newsId_arr = [];

            // dd($detach_similar_newsId[0]->news_id);

            foreach ($detach_similar_newsId as $newsItem) {
                $similar_newsId_arr[] = $newsItem->similar_news_id;
            }

            // dd($similar_newsId_arr);

            $result = array_diff($similar_newsId_arr, $request->input('news_id'));

            foreach ($result as $delItem) {
                DB::table('similar_news')
                                    ->where('news_id', $data->id)
                                    ->where('similar_news_id', $delItem)
                                    ->delete();
            }
            // dd($result);

             foreach ($request->input('news_id') as $key => $value) {

                    $item = DB::table('similar_news')
                                     ->select()
                                     ->where('similar_news_id', $value)
                                     ->where('news_id', $data->id)
                                     ->get();
             // dd($item);
                    if(count($item) !== 0) {
                        ...
                        DB::table('similar_news')
                                    ->insert(
                                    ['news_id' => $data->id, 'similar_news_id' => $value]
                        );
                    }
                }

        } else {
            DB::table('similar_news')->where('news_id', $data->id)->delete();
        }
        ...
        $similar_array = $request->news_belongstomany_news_relationship;
    ...
    public function create(Request $request)
    {
        ...
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'all_news'));
    }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        DB::table('similar_news')->where('news_id', $id)->delete();
